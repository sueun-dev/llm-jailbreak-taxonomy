...
```
(Carlini et al. 2023, GPT-3.5 — 이미 패치됨. 패턴 예시용)
...
## BB. 공급망
```
npm i @anthropicai/sdk    ← 오타 스쿼팅. 진짜는 @anthropic-ai/sdk
pip install langshain      ← 유사 이름
...
## HH. 모델 내부
```
" SolidGoldMagikarp" → GPT 모델 이상 동작 (글리치 토큰, 2023 발견)
200k context 중 78% 지점 지시 → "lost-in-the-middle" 망각
```

## II. 방어 공격 (Meta)
```
Llama Guard에 동일 jailbreak 기법 적용.
방어 모델도 LLM = 같은 공격 표면.
```
